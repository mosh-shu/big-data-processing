...
for file in files:
    ...
    for row in f:
        if row == []:
            continue
        out = [getDegree(row[0][0:8]), getDegree(row[0][8:16]), getISO8601(row[0][16:30]), \
            str(int(row[0][30:34], 16)), str(int(row[0][34:38], 16)), str(int(row[0][38:40], 16)), \
            str(int(row[0][40:44], 16)), str(int(row[1], 16)), str(int(row[2][0:2], 16)) \
            ]
        outs.append(out)
